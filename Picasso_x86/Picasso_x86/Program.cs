...
namespace Picasso
{
    static class Program
    {
        ...
        {
        	StringBuilder meus_dados_i = new StringBuilder(1027);
        	
			Alladin_converter.ControlName(meus_dados_i);
						
			Alladin_converter.name = meus_dados_i.ToString();
            Application.EnableVisualStyles();
            Application.SetCompatibleTextRenderingDefault(false);
            if(args.Length == 0)
            Application.Run(new selector(""));
            else
            	Application.Run(new converter2(args[0]));
